...
Objective: In this assignment, we extend a classical predator-prey model to include conditional hunting/harvesting (in our model we use, synonymous term, "hunting"). The goal is to examine how different hunting intensities and rules (e.g., only hunt when prey exceed a certain threshold) affect the long-term stability of the ecosystem. This approach helps identify sustainable harvesting strategies that protect both predator and prey populations over time.

## Part 1: Build the Hunting Model
...
## Part 2: Hunting Effects on Stability

## Source the Data

```{r}
source(here("R/hunting-lotvmod.R"))
hunting-lotvmod
```
...
results <- pmap_dfr(grid, function(hX, min_harvest_X) {
  ...
  out <- ode(y = state, times = times, func = complex_pred_prey, parms = parms)
...
A simulation was considered stable if both prey and predator populations remained above 50 individuals at the end of 50 years. This metric was chosen because it is biologically meaningful and interpretable. It is biologically meaningful because populations below 50 individuals are at high risk of extinction due to demographic or environmental stochasticity. It is simple and interpretable, which allows for clear comparisons across harvesting strategies. This threshold approach avoids false confidence in highly fluctuating but non-zero population dynamics and provides a conservative estimate of sustainability.
